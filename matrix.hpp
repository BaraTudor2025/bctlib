...
namespace
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            MatrixConstIter(const iterator_row Rit, const iterator_col Cit) : row_(Rit), curr_(Cit) { }
     ...
     *   In Ctor se ia marimea matricii, elementele sunt initializate cu 0 daca nu se specifica val de initializare
     *   Ctor-ul poate fi omis dar matricea trebuie initializata prin functia membru resize() sau prin citirea ei dintr-un fisier
     *   Elementele pot fi accesate prin intermediul operatorul [0][0] / (0,0) sau prin functia prieten for_each(*this,Func),
    ...
    */
    ...
    class Matrix {
        ...
        public:
            ...
            using iterator_row               = detail::RowIteratorAdaptor <typename matrix_t::iterator>;
            using const_iterator_row         = detail::RowIteratorAdaptor <typename matrix_t::const_iterator>;
            using reverse_iterator_row       = detail::RowIteratorAdaptor <typename matrix_t::reverse_iterator>;
            using const_reverse_iterator_row = detail::RowIteratorAdaptor <typename matrix_t::const_reverse_iterator>;
        ...
        public:
            ...
            Matrix(const Matrix& other, std::string name)
                : Matrix(other) { _name = name; }
            ...
            reverse_iterator
            rbegin() noexcept
            { return reverse_iterator(iterator(_matrix.end()-1,
                                              (_matrix.end()-1)->end())); }
            const_reverse_iterator
            rbegin() const noexcept
            { return const_reverse_iterator(const_iterator(_matrix.cend()-1,
                                                          (_matrix.cend()-1)->cend())); }
            reverse_iterator
            rend() noexcept
            ...
            iterator_row
            begin1() noexcept
            { return iterator_row(_matrix.begin()); }

            const_iterator_row
            begin1() const noexcept
            { return const_iterator_row(_matrix.cbegin()); }

            iterator_row
            end1() noexcept
            { return iterator_row(_matrix.end()); }
            ...
            reverse_iterator_row
            rbegin1() noexcept
            { return reverse_iterator_row(_matrix.rbegin()); }

            const_reverse_iterator_row
            rbegin1() const  noexcept
            ...
            const_reverse_iterator_row
            rend1() const noexcept
            { return const_reverse_iterator_row(_matrix.crend()); }


            const_iterator_row
            cbegin1() const noexcept
            { return const_iterator_row(_matrix.cbegin()); }

            const_iterator_row
            cend1() const noexcept
            { return const_iterator_row(_matrix.cend()); }

            const_reverse_iterator_row
            crbegin1() const  noexcept
            { return const_reverse_iterator_row(_matrix.crbegin()); }

            const_reverse_iterator_row
            crend1() const noexcept
            { return const_reverse_iterator_row(_matrix.crend()); }


            proxy
            operator[] (size_type line) noexcept
            { return proxy(_matrix[line]); }

            const_proxy
            operator[] (size_type line) const noexcept
            { return const_proxy(_matrix[line]); }

            reference
            operator()(size_type line, size_type coll)
            { return _matrix.at(line).at(coll); }

            const_reference
            operator()(size_type line, size_type coll) const
            { return _matrix.at(line).at(coll); }
            ...
            const_reference
            at(size_type line ,size_type coll) const
            { return _matrix.at(line).at(coll); }

            reference
            front() noexcept
            { return _matrix.front().front(); }
            ...
            void zero_is_element(bool ok) noexcept
            { _zero_is_elem = ok; }

            bool is_zero_elem() const noexcept
            { return _zero_is_elem; }
            ...
            detail::proxy_print_info<T>
            info() const noexcept
            { return detail::proxy_print_info<T>(*this); }

            Matrix&
            fill_line(size_type index, value_type value) noexcept
            {
            ...
            Matrix&
            fill_coll(size_type index, value_type value) noexcept
            {
                for(auto& vec : _matrix)
                    vec.at(index) = value;
                return *this;
            ...
            Matrix&
            insert_line(iterator_row pozitie)
            {
                _line++;
            ...
            Matrix&
            erase_line(iterator_row  pozitie)
            {
                _line--;
            ...
            {
                _line -= std::distance(first.row().data(),
                                       last.row().data());
                auto distanta = std::distance(first.row().begin(),
                                              first.curr()-1);
                std::fill(first,last,0);
                auto it = _matrix.erase(first.row().data(),
                                        last.row().data());
            ...
            friend std::basic_istream<TChar,CharTraits>&
            operator>>(std::basic_istream<TChar,CharTraits>& stream, Matrix<Tp>& matrix);

        private:

            // verifica daca exista linii "moarte"(linii care au doar elem de 0) in matrice
            void _clean_end()
            {
    ...
    Matrix<T>::get_output_width() const
    {
    ...
    void Matrix<T>::remove_if(Func conditie)
    {
        ...
        size_type _size = 0;
        while(_size * _size < filter_vec.size())
            _size++;
    ...
    template <typename T>
    inline Matrix<T>
    operator+ (Matrix<T> lhs, Matrix<T> const& rhs)
    {
        lhs += rhs;
        return lhs;
    }
    ...
    std::basic_istream<TChar,CharTraits>&
    operator>>(std::basic_istream<TChar,CharTraits>& stream, Matrix<T>& matrix)
    {
        if(matrix.empty())
        {
            T value;
            ...
            while( getline(stream, line) )
            {
                ...
                while(split >> value)
                    matrix.data().back().push_back(value);
            }

            std::vector<T> min_max_vec;
            for(auto& vec : matrix.data())
                min_max_vec.push_back(vec.size());

            using size_type = typename Matrix<T>::size_type;
            size_type _min = *std::min_element(min_max_vec.begin(),
                                               min_max_vec.end());
            size_type _max = *std::max_element(min_max_vec.begin(),
                                               min_max_vec.end());
            if(_min != _max)
                for(auto& vec : matrix.data())
                    while(vec.size() < _max)
    ...
    template <typename T, typename TChar, typename CharTraits>
    std::basic_ostream<TChar,CharTraits>&
    operator<<(std::basic_ostream<TChar,CharTraits>& stream, const Matrix<T>& matrix)
    {
        if(!matrix.empty()){
            auto _output_width = matrix.get_output_width();

            stream << '\n' << " Matricea " << matrix.name() << ": " << matrix.line() << " x " << matrix.coll() << '\n';
    ...
    template <typename T, typename TChar, typename CharTraits>
    std::basic_ofstream<TChar,CharTraits>&
    operator<<(std::basic_ofstream<TChar,CharTraits>& file, const Matrix<T>& matrix)
    {
        if(!matrix.empty()) {
            auto _output_width = matrix.get_output_width();
            for(auto& vec : matrix.data()) {
                for(auto& elem : vec)
                    file << std::setw(_output_width) << elem;
                file << '\n';
            }
            file << '\n';
        }
    ...
    template <typename T, typename TChar, typename CharTraits>
    std::basic_ostream<TChar,CharTraits>&
    operator<<(std::basic_ostream<TChar,CharTraits>& stream, const detail::proxy_print_info<T> info)
    {
